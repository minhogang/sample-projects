...
class square:
    ...
    def __init__(self, length):
        ...
        self.registerclickcheck = 2
        self.registerclickcheck1 = 1
        self.piece_inside_highlighted = None
    def draw_pieces(self):
        for images in self.blackdictionary.values():
            images.draw(win)
        for images in self.whitedictionary.values():
            images.draw(win)
    def draw_board(self):
    ...
    def register_click(self):
        while self.registerclickcheck % 2 != 0:
            ...
            def highlight_square(self):
                ...
                def register_click_and_move_piece(self):
                        ...
                        if not self.piece_inside_highlighted == None:
                            ...
                            self.registerclickcheck += 1
                            self.registerclickcheck1 += 1
                register_click_and_move_piece(self)
                self.piece_inside_highlighted = None
                highlightedrect.undraw()
            if self.registerclickcheck % 2 != 0:
                highlight_square(self)
            else:
                return
    def register_click1(self):
        while self.registerclickcheck1 % 2 != 0:
            clickcoords = win.getMouse()
            if clickcoords.x > self.boardwidth or clickcoords.y > self.boardlength:
                win.close()
                break
            ...
            def highlight_square(self):
                ...
                highlightedrect.setOutline("yellow")
                highlightedrect.draw(win)
                for i in self.whitedictionary.keys():
                    if i.x == highlightedrect.getCenter().x and i.y == highlightedrect.getCenter().y:
                        self.piece_inside_highlighted = i
                        break
                ...
                def register_click_and_move_piece(self):
                        ...
                        if not self.piece_inside_highlighted == None:
                            ...
                            newcoords = (nearest_coords_points.x, nearest_coords_points.y)
                            dxdy = Point(nearest_coords_points.x - self.piece_inside_highlighted.x, nearest_coords_points.y - self.piece_inside_highlighted.y)
                            for i in [self.whiterook1, self.whiterook2]:
                                if self.piece_inside_highlighted == i:
                                    if dxdy.x != 0:
                                        if dxdy.y != 0:
                                            return
                                    ...
                                    if dxdy.x == 0:
                                        if dxdy.y > 0:
                                            for key in self.whitedictionary.keys():
                                                if int(key.y) in range(int(i.y), int(i.y+dxdy.y)) and key.x == i.x:
                                                    if key == i:
                                                        continue
                                                    if dxdy.y + i.y > key.y:
                                        ...
                                        if dxdy.x < 0:
                                            for key in self.whitedictionary.keys():
                                                ...
                                                if key.x in range(int(dxdy.x + i.x), int(i.x)) and key.y == i.y:
                                                    if dxdy.x + i.x < key.x:
                                                        return
                            for i in [self.whitebishop1, self.whitebishop2]:
                                if self.piece_inside_highlighted == i:
                                    if dxdy.x == 0:
                                        return
                                    if dxdy.y == 0:
                                        ...
                                        for key in self.whitedictionary.keys():
                                            ...
                                            if dxdy.x < 0 and dxdy.y < 0:
                                                if key.x in range(int(i.x + dxdy.x)+1, int(i.x)) and key.y in range(int(dxdy.y + i.y)+1, int(i.y)) and abs(key.x - i.x) == abs(key.y - i.y):
                                                    if i.y + dxdy.y < key.y:
                                                        return
                            for i in [self.whiteknight1, self.whiteknight2]:
                                if self.piece_inside_highlighted == i:
                                    if abs(dxdy.x) + abs(dxdy.y) != self.length * 3:
                                        return
                            for i in [self.whitequeen]:
                                if self.piece_inside_highlighted == i:
                                    if dxdy.x != 0 and dxdy.y != 0:
                                        if abs(dxdy.x) != abs(dxdy.y):
                                            return
                                        ...
                                        for key in self.whitedictionary.keys():
                                            ...
                                            if dxdy.x < 0 and dxdy.y < 0:
                                                if key.x in range(int(i.x + dxdy.x)+1, int(i.x)) and key.y in range(int(dxdy.y + i.y)+1, int(i.y)) and abs(key.x - i.x) == abs(key.y - i.y):
                                                    if i.y + dxdy.y < key.y:
                                                        return
                            for i in [self.whiteking]:
                                if self.piece_inside_highlighted == i:
                                    if abs(dxdy.x) > self.length or abs(dxdy.y) > self.length:
                                        return
                            for i in [self.whitepawn1, self.whitepawn2, self.whitepawn3, self.whitepawn4, self.whitepawn5, self.whitepawn6, self.whitepawn7, self.whitepawn8]:
                                if self.piece_inside_highlighted == i:
                                    if self.whitepawninitpositionstatus[i] == 0:
                                        if dxdy.x != 0:
                                            return
                                        if dxdy.y > 0:
                                            return
                                        if abs(dxdy.y) > self.length * 2:
                                            return
                                        for key in self.whitedictionary.keys():
                                            if key == i:
                                                continue
                                            if key.y in range(int(i.y), int(i.y + dxdy.y)) and key.x == i.x:
                                                if i.y + dxdy.y > key.y:
                                                    return
                                        self.whitepawninitpositionstatus[i] = 1
                                        break
                                    if self.whitepawninitpositionstatus[i] == 1:
                                        if dxdy.x != 0:
                                            return
                                        if dxdy.y > 0:
                                            return
                                        if abs(dxdy.y) > self.length * 1:
                                            return

                            image = self.whitedictionary[self.piece_inside_highlighted]
                            imageanchor = image.getAnchor()
                            for key in self.whitedictionary.keys():
                                if imageanchor.x + dxdy.x == key.x and imageanchor.y + dxdy.y == key.y:
                                    return
                ...
                self.piece_inside_highlighted = None
                highlightedrect.undraw()
            if self.registerclickcheck1 % 2 != 0:
                highlight_square(self)
            else:
                return
